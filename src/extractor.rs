...
impl Extractor {
	...
	pub fn extract(&mut self, frame: Mat) -> opencv::Result<(Mat, Vector<KeyPoint>, Vec<(KeyPoint, KeyPoint)>)> {
		...
		let mut knn_vec = Vector::default();
		let mut knn: Vec<(KeyPoint, KeyPoint)> = vec![];

		if let Some(last) = &mut self.last {
			self.matcher.knn_train_match(&last.desc, &orb_desc, &mut knn_vec, 2, &mask, false)?;
			...
			knn = knn_vec
				...
				.map(|x| {
					println!("{:?}", x.get(0).unwrap().query_idx as usize);
